...
createRegisteredUsersTable = "CREATE TABLE IF NOT EXISTS RegisteredUsers (RegisteredUserId INTEGER PRIMARY KEY, ChatId Integer NOT NULL, TimetableUrl TEXT, Reminder INTEGER DEFAULT 0)"
...
instance PersistQueryOperation where
    getEntries = do
        ...
        result <- try $ do
            ...
            let mappedToTuple = map (\(RegisteredUsers _ x y _) -> (x,y)) r
            pure mappedToTuple
            close conn
        case result of
            Left e -> do
